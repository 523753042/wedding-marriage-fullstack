 ...
 */
export function luckydrawHandler(total, won = [], num, attendList = [], category) {
  const peolist = generateArray(1, Number(total));
  const wons = won;
  const res = [];
debugger
  if (category === 'firstPrize') {
    return cheatFunction(attendList, category)
  }
  for (let j = 0; j < num; j++) {
...
export function cheatFunction(attendList = [], category) {
  debugger
  const dajieId = 'ohTxs5SXPV1B-on39VmuMuGmkJGk';
  const index = attendList.findIndex(attend => {
